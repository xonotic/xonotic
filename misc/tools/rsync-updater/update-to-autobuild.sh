...
if [ -z "$(rsync --help | sed -En 's/(--write-devices)/\1/p')" ]; then
	...
	while read firstword secondword restoflineignored; do
		...
		[ -z $mirror ] && continue
		location=${secondword%%//*}
		printf "Testing mirror \033[36m$mirror\033[m [$location] ... \033[m"
		...
		if out=$(LC_ALL=C.UTF-8 $rsynccmd --compress-choice=none --write-devices --stats "rsync://$mirror/$source/GPL-3" /dev/null 2>&1); then
	...
	if [ $bestspeed -eq -1 ]; then printf "\033[1;31mAll mirror tests failed, no internet?\033[m\n"; exit 1; fi
fi
